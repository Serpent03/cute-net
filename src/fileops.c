...
void 
*read_section(size_t size, FILE *fptr, uint32 *cursor, uint32 *length) {
  ...
  while (memcmp(cmp, magic, sizeof(magic)) != 0) {
    /* the length of the section is determined by difference 
     * in the starting position of the cursor(old_cursor) and
     * when it finally reaches a 4-byte block of value 55|AA|00|00 */
    fread(cmp, sizeof(uint8), 4, fptr);
    (*cursor)++;
  }
  /* reset the cmp array so that on the next run it will again 
   * try to find end of section. */
  cmp[0] = 0; 
  int32 size_of_section = (((*cursor) - 1) - old_cursor) * 4;
  *length = size_of_section / size;
  /* the difference in the cursor position gives us the number 
   * of bytes that a specific section encases. after we get this, 
   * we can get the size of the buffer itself by dividing that with
   * the<<size>> of the data, which we are conveniently providing! 
   */

  void *buffer = (void*)malloc((*length) * size); 
